...
with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
    ...
    with tf.variable_scope('loss'):
        ...
        env.loss_pgd = 0.5*(env.loss+env.loss_adv)


    with tf.variable_scope('train_op'):
        optimizer = tf.train.AdamOptimizer()
        env.train_op = optimizer.minimize(env.loss)
        env.train_op_Adv = optimizer.minimize(env.loss_pgd)
...
with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
    ...
    env.x_pgd = pgd(model, env.x, epochs=env.pgd_epochs, eps=env.pgd_eps)
    env.ybarAdv, logitsAdv = model.model(env.x_pgd, logits=True, training=env.training)
...
def make_pgd(sess, env, X_data, epochs=1, eps=0.01, batch_size=128):
    """
...
#
# np.save('npy/pgd_train', X_adv)
X_adv = np.load('npy/pgd_train.npy')

print('\nTraining')
train(sess, env, X_train, y_train, X_valid, y_valid, X_adv, load=False, epochs=12,
      name='pgd0')

print('\nGenerating adversarial model')
X_adv1 = make_pgd(sess, env, X_test, eps=0.02, epochs=5)
